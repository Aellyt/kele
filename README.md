## 说明
一些脚本，环境变量chinaUnicomCookie。抽奖控制变量draw_lottery填任意值自动抽奖，不填不抽，默认不抽奖。阅读积分环境变量PHONE_NUM多账户用&分割。

尽量在家宽环境跑，不建议使用云服务器，尤其是某企鹅厂的，被屏蔽很多。

测试环境为十个号，再多账号导致需要代理，自行解决，非必要不会修改支持。
